...
execute as @s[tag=pcm2_mvalid,nbt={Item:{tag:{pcm2_unusualifier:1}}}] if entity @e[type=item,nbt={Item:{tag:{pcm2_modified:1,pcm2_hat:1,pcm2_unusual:1}}},distance=..0.5,limit=1] run advancement grant @p[distance=..5] only pcm_accoutrements:accoutrements/personal_touch
