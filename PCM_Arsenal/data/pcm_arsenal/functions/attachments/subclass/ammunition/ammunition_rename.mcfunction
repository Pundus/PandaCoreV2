...
#quick tag
tag @s add ARS_ATCH_AmmoT_Meta

#the name
data modify block ~1 0 ~1 Text1 set value '[{"translate": "arsenal.item.attachments.ammotype","color": "gray","italic": false},{"nbt":"Inventory[{Slot:-106b}].tag.display.Name","entity":"@a[limit=1,tag=ARS_ATCH_AmmoT_Meta,sort=nearest]","interpret":true}]'

#apply
data modify block ~ 0 ~ Items[0].tag.display.Lore[20] set from block ~1 0 ~1 Text1

#remove tag
tag @s remove ARS_ATCH_AmmoT_Meta
