...
#if gun is half full
execute as @s[predicate=!pcm_arsenal:offhand_item/ammo_0_off] unless score @s ARS_RndsInOH = %P90 ARS_SET_Rounds at @s if score @s ARS_ReloadProg > %P90 ARS_SET_ReloadT run function pcm_arsenal:reload/pdw/p90/override/or_p90_reload_semi

execute as @s[predicate=pcm_arsenal:offhand_item/duplicator,predicate=!pcm_arsenal:offhand_item/ammo_0_off] if score @s ARS_RndsInOH = %P90 ARS_SET_Rounds at @s if score @s ARS_ReloadProg > %P90 ARS_SET_ReloadT run function pcm_arsenal:reload/pdw/p90/override/or_p90_reload_semi
