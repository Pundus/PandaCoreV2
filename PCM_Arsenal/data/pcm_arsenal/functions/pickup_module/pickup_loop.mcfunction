...
execute as @e[type=item,tag=ARS_PickupLocked] at @s if entity @a[predicate=pcm_arsenal:sneaking,distance=..1.5,scores={ARS_WeightF=..99}] run function pcm_arsenal:pickup_module/unlock_items

#If overridden, and has the special tag:
execute as @e[type=item,nbt={Item:{tag:{ars_instapick:1b}}}] at @s if entity @a[predicate=pcm_arsenal:sneaking,distance=..1.5,scores={ARS_WeightF=..99}] run function pcm_arsenal:pickup_module/unlock_items
